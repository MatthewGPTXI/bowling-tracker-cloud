...
(async () => {
  ...
  assert(row('Closed frame %').includes('70.0%')); assert(row('Closed frame %').includes('80.0%'));
  assert(!$('shareFriendComparison').hidden); assert(!$('shareFriendComparison').disabled);
  const card = friends.getComparisonCardData();
  assert.equal(card.kind, 'comparison'); assert.equal(card.bowlers.length, 2);
  assert.equal(card.bowlers[0].name, app.getProfileName());
  assert.equal(card.bowlers[0].average, 180, 'Card ignores active personal filters');
  assert.equal(card.bowlers[1].name, friend.displayName);
  assert.equal(card.bowlers[1].closedFramePct, 80);
  assert.equal(card.bowlers[1].frameCount, 20, 'Older summaries with recorded details remain supported');
  assert(!/"(?:uid|groupId|email|notes|ball|alley)":/.test(JSON.stringify(card)));
  card.bowlers[1].average = 1;
  assert.equal(friends.getComparisonCardData().bowlers[1].average, 200, 'Card is an independent snapshot');
  ...
  assert.equal(friends.getComparisonCardData().bowlers[1].closedFramePct, null, 'Stale details cannot imply closed frames');
  ...
  assert.equal(friends.getComparisonCardData().bowlers[1].closedFramePct, null);
  const partial = { ...friend, frameStatsGames: 5, scoreOnlyGames: 15, frameStatsUpdatedAt: friend.updatedAt };
  friends.setMembers([me, partial], context);
  assert.equal(friends.getComparisonCardData().bowlers[1].frameCount, 5);
  assert.equal(friends.getComparisonCardData().bowlers[1].closedFramePct, 80, 'Only detailed games are in the closed-frame denominator');
  friends.setMembers([me, { ...partial, frameStatsGames: 0, scoreOnlyGames: 20 }], context);
  const scoreOnly = friends.getComparisonCardData().bowlers[1];
  for (const key of ['strikePct', 'closedFramePct', 'cleanGames', 'totalStrikes']) assert.equal(scoreOnly[key], null);
  assert.equal(scoreOnly.average, 200);
  friends.setMembers([me, { ...friend, noTapGames: 2 }], context);
  assert($('shareFriendComparison').disabled);
  assert.equal(friends.getComparisonCardData(), null, 'Unsafe legacy no-tap summaries cannot be exported');
  friends.setMembers([me, { ...partial, frameStatsUpdatedAt: 999 }], context);
  assert.equal(friends.getComparisonCardData(), null, 'Unsafe legacy frame summaries cannot be exported');
  friends.setMembers([me, friend], context);
  ...
  assert($('shareFriendComparison').hidden); assert.equal(friends.getComparisonCardData(), null);
  friends.open('b');
  ...
  assert($('shareFriendComparison').disabled); assert.equal(friends.getComparisonCardData(), null);
  ...
  assert.equal(friends.getComparisonCardData().bowlers[1].highSeries, 0);
  friends.clear();
