...
function element(id='') {
 ...
 const e={id,value:'',textContent:'',dataset:{},hidden:false,open:false,children:[],listeners:{},attributes:{},classList:{add:x=>classes.add(x),remove:x=>classes.delete(x),contains:x=>classes.has(x),toggle:(x,yes)=>yes===undefined?(classes.has(x)?classes.delete(x):classes.add(x)):(yes?classes.add(x):classes.delete(x))},addEventListener(type,fn){(this.listeners[type]??=[]).push(fn)},async fire(type,event={}){for(const fn of this.listeners[type]||[])await fn({target:this,preventDefault(){},...event})},focus(){this.focused=true},scrollIntoView(){},removeAttribute(name){delete this.attributes[name]},setAttribute(name,v){this.attributes[name]=v},close(){this.open=false},showModal(){this.open=true},appendChild(child){this.children.push(child);child.parent=this},remove(){this.parent.children=this.parent.children.filter(x=>x!==this)},querySelector(sel){if(sel==='legend')return this.legend??=element();if(sel==='.remove-series-row')return this.removeBtn??=element();if(sel==='input')return Object.values(this.fields||{})[0];return this.fields?.[sel.match(/data-field="(\w+)"/)?.[1]]},querySelectorAll(sel){if(sel==='input'){if(this.fields)return Object.values(this.fields);return this.children.flatMap(c=>c.querySelectorAll(sel));}if(sel==='[data-field="score"]')return this.children.map(c=>c.fields.score);return []}};
 let html='';Object.defineProperty(e,'innerHTML',{get:()=>html,set(v){html=v;e.children=[];if(v.includes('data-field="score"')){e.fields={};for(const field of ['score','openFrames','strikes','strikeOpp','notes']){e.fields[field]=element();e.fields[field].value=field==='strikeOpp'?'10':'';}}}});return e;
}
...
(async()=>{
 ...
 t.openSessionEditor('2026-08-16|||league');$('editSessionName').value='Renamed';await t.saveSessionEdit(submit);assert.equal(app.getGames().filter(g=>g.sessionName==='Renamed').length,4);
